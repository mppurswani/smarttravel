**SmartTravel 🌍 (Production-Ready)**
Production-ready full-stack Java Spring Boot application exploring 50+ Indian cities with culture, food, hidden gems, and personalized travel recommendations. Currently 12 cities preloaded, scalable to 50+ via admin panel.

**✨ Features**
JWT Authentication — Register/login, BCrypt passwords, role-based access (USER/ADMIN)

**Favourites System** —
Add/remove cities per user (fully private/isolated)
...
JwtAuthenticationFilter → SecurityConfig → Role-based access
     ↓
GlobalExceptionHandler → ResourceNotFoundException → HTTP codes
     ↓
...
Note: 12 cities preloaded; supports 50+ cities via admin panel.
